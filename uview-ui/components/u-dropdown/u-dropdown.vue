...
<script>
	...
	export default {
		...
		methods: {
			...
			// 点击菜单
			menuClick(index) {
				this.contentHeight = this.contentHeightCalc; // 解决置顶的时候因为Z-index挡住下面的内容
				// 判断是否被禁用
			...
			// 设置下拉菜单处于收起状态
			close() {
				this.contentHeight = 0; // 解决置顶的时候因为Z-index挡住下面的内容
			...
			getContentHeight() {
				...
				this.$uGetRect('.u-dropdown__menu').then(res => {
					...
					this.contentHeightCalc = windowHeight - res.bottom; // 解决置顶的时候因为Z-index挡住下面的内容
				})
			}
		}
	}
