...
MpHtmlParser.prototype.popNode = function(node) {
	...
	var attrs = node.attrs;
	// 替换一些标签名
	if (cfg.blockTags[node.name]) node.name = 'view';
	else if (!cfg.trustTags[node.name]) node.name = 'span';
	// 去除块标签前后空串
	if (node.name == 'view' || node.name == 'p' || node.name[0] == 't') {
	...
	// 自动压缩
	if (node.name == 'view' && !Object.keys(attrs).length && childs.length == 1 && childs[0].name == 'view')
