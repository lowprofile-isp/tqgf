	...
	export default {
		...
		methods: {
			...
			setContent(html, append) {
				...
				if (append)
					this.$refs.web.evalJs('var b=document.createElement(\'view\');b.innerHTML=\'' + html.replace(/'/g, '\\\'') +
					...
					html =
						...
						this.domain + '"><view id="parser"' + (this.selectable ? '>' : ' style="user-select:none">') + this._handleHtml(html).replace(/\n/g, '\\n') +
						'</view><script>"use strict";function e(e){if(window.__dcloud_weex_postMessage||window.__dcloud_weex_){var t={data:[e]};window.__dcloud_weex_postMessage?window.__dcloud_weex_postMessage(t):window.__dcloud_weex_.postMessage(JSON.stringify(t))}}' +
				...
				this.$refs.web.evalJs(
					...
					(this.autoscroll ? 'for(var p,w=document.getElementsByTagName("table"),T=0;p=w[T];T++){var E=document.createElement("view");E.style.overflow="scroll",p.parentNode.replaceChild(E,p),E.appendChild(p)}' : '') +
				...
				if (!append) {
					...
					if (!this.rtf) this.rtf = view;
					else this.rtf.appendChild(view);
				}
				view.innerHTML = this._handleHtml(html, append);
				...
				if (this.autoscroll) {
					...
					for (var table of tables) {
						...
						table.parentNode.replaceChild(view, table);
			...
			getText(ns = this.nodes) {
				...
				for (var i = 0, n; n = ns[i++];) {
					if (n.type == 'text') txt += n.text.replace(/&nbsp;/g, '\u00A0').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
						...
						// 块级标签前后加换行
						var block = n.name == 'p' || n.name == 'view' || n.name == 'tr' || n.name == 'li' || (n.name[0] == 'h' && n.name[1] >
							'0' && n.name[1] < '7');
