<template>
	<view class="errorPush-page">
		...
		<view class="container">
			<view class="content">
				<scroll-view class="material-list" :class="!error_id && stateFunc(baseFormData.state).text?'edit':''" scroll-y :scroll-top="scrollTop">
					<card-info title="基本信息">
						<template v-slot:head-right>
							<u-tag v-if="baseFormData.state" :text="stateFilter(baseFormData.state).text || ''"
								:type="stateFilter(baseFormData.state).type" />
						</template>
						<uni-forms class="content-form" :modelValue="baseFormData" :rules="rules">
		...
		<view class="btn-group" v-if="!error_id && stateFunc(baseFormData.state).text">
			<u-button v-if="!error_id" type="primary" :loading="btnLoading" hover-class="none"
				@click="btnClick">提交</u-button>
			<u-button v-else-if="stateFunc(baseFormData.state).text" type="primary" :loading="btnLoading"
				hover-class="none" @click="doneClick">{{stateFunc(baseFormData.state).text}}</u-button>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			doneClick() {
				let stateInfo = this.stateFunc(this.baseFormData.state);
				if (stateInfo.func) {
					api.web.dataset.call_kw({
						model: 'mrp.abnormal',
