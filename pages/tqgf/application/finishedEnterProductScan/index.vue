	...
	<view class="finishedProductScan-page" :style="{height: `calc(100vh - ${$mStore.state.navBarHeight}px)`}">
			...
			<view class="content">
				...
				<scroll-view class="material-list" scroll-y :scroll-top="scrollTop" @scroll="scrollTap">
					...
					<uni-forms v-else class="material-item" v-for="(item,index) in material_list" :key="index"
						...
						<uni-forms-item label="质检结果">
							<span
								:style="{color: checkResultFilter(item.check_result)}">{{item.check_result || ''}}</span>
						</uni-forms-item>
		...
		<view>
			<view class="btn-group">
				...
				<u-button type="primary" :loading="btnLoading" :disabled="!material_list.length"
					@click="postEquipmentInfo">入 库</u-button>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async scanClick() {
				...
				[
					'HQ20240913ZC01/HQ320XBA-24-1012-001'
				]
				.forEach((item) => {
