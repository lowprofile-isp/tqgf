			...
			<view class="content">
				<scroll-view class="material-list" scroll-y :scroll-top="scrollTop">
					<uni-forms ref="baseForm" class="material-item" :modelValue="baseFormData" :rules="rules"
						label-width="70">
						<uni-forms-item label="异常标题" name="name" required class="require-item">
						...
						<uni-forms-item label="备注" name="note">
							<uni-easyinput v-model="baseFormData.note" type="textarea" autoHeight
								:styles="easyinputStyles" :disabled="error_id ? true : false" clearSize="22"
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				this.uploadGetResultInfo(null, (result) => {
					if (result) {
						...
						this.tagList.forEach(t => t.disabled = true);
						this.uploadInfo = {
							...result
						};
			...
			uploadGetResultInfo(params, callback) {
				...
				payload['responsible_user_id'] = this.selectUser?.length ? this.selectUser.map(u => u.id)[0] : null;
				...
				if (res?.length) {
					this.tagList = res.map(t => {
						return {
							text: t.name,
							value: t.id,
							disabled: this.error_id ? true : false
						}
					})
				}
			},
