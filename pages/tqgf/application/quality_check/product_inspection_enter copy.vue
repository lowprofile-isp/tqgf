<template>
	<view class="product_inspection">
		...
		<page-loading :loadmore="loadmore" @refresh="refresh">
					...
					<u-tr style="height: auto;">
						...
						<u-td style="flex: auto; flex-grow: 1; padding: 0; border-bottom: 0; border-right: 0;">
							<u-tr v-for="(item,index) in inspection_list" :key="index">
								<template v-if="production_obj.product_category === '刺针'">
									<u-td v-if="index === inspection_list.length - 1"
										style="width: 200rpx; flex: none; font-size: 28rpx; font-weight: bold;">检验结果</u-td>
									<u-td v-else
										style="width: 200rpx; flex: none; font-size: 36rpx; font-weight: bold;">{{index + 1}}</u-td>
								</template>
								<template v-else>
									<u-td
										style="width: 200rpx; flex: none; font-size: 40rpx; font-weight: bold;"><u-input
											type="text" :value="'*'" disabled input-align="center"
											placeholder="" /></u-td>
								</template>
								<u-td :width="!isBespread ? '300rpx' : 'auto'" class="category-cell"
									...
									<template v-if="i.type.indexOf('检验结果') !== -1 && i[i.type] !== null">
										<u-input v-model="i[i.type]" type="select" input-align="center"
											placeholder="请选择" @click.native="handleFocus3(index, index2)" />
									</template>
									<template v-else-if="i.type_value === 'Measure'">
			...
			<view style="padding: 12rpx 20rpx; text-align: right;">
				<view class="add-btn">
					<u-icon name="plus" color="#fff" size="28" @click="addCellClick"></u-icon>
				</view>
				<view class="del-btn" :style="{opacity: inspection_list.length <= 1 ? .6 : 1}">
					<u-icon name="minus" color="#fff" size="28" @click="delCellClick"></u-icon>
				</view>
			</view>
			<u-table :style="iskeyOpen ? hideStyle : {}" v-if="true" style="position: fixed; left: 0; bottom: 152rpx;">
				<u-tr style="height: 100rpx;">
					...
					<template v-if="isEdit">
						<u-td style="font-weight: bold;font-size: 32rpx; color: #333;">检验日期</u-td>
						<u-td>
							<input :value="inspector_date" disabled @input="inputTextChange"
								@blur="(event) => inputTextBlur2(event)" type="text" input-align="center"
								:clearable="false" />
						</u-td>
					</template>
			...
			<u-action-sheet :list="checkResultSelect" v-model="checkResultToolFlag" :safe-area-inset-bottom="true"
				cancel-text="清除选择" @click="selectCheckResult" @close="closeCheckResult"></u-action-sheet>
...
<script>
	...
	export default {
		...
		methods: {
			...
			inputTextBlur3(event) {
				this.inspector_date = event.detail.value
			},
			...
			async getResultInfo() {
				let res = await api.web.dataset.call_kw({
					model: 'stock.picking',
					method: 'get_quality_checks_by_picking',
					args: [
						[]
					],
					kwargs: {
						picking_id: this.production_obj.id
					}
				})
				if (res) {
					this.checkResultInfo = res;
				}
			},
			async getCheckList() {
				let inspection_list = [];
				...
				const res = await Model.search_read({
					domain: [
						['picking_id', '=', this.production_obj.id],
						['quality_point_type', '=', 'incoming_check'],
					...
					for (let i = 0; i < max; i++) {
						...
						for (let j = 0; j < this.category_list.length; j++) {
							...
							arr.push({
								...
								point_id: this.category_list[j].point_id,
							})
						}
						inspection_list.push(arr)
					}
					this.inspection_list = [...inspection_list];
					if (this.production_obj.product_category === '刺针') {
						await this.getResultInfo();
						let name = '检验结果';
						let arr = this.inspection_list[this.inspection_list.length - 1].map(i => {
							let checkResultValue = this.checkResultInfo[i.point_id[0]];
							this.inspector_user = checkResultValue.check_inspector || '';
							this.inspector_date = checkResultValue.control_date || '';
							return {
								point_id: i.point_id,
								initial_value: '',
								title: name,
								type: name,
								type_value: 'Instructions',
								[name]: i.point_id[0] == 127 ? null : checkResultValue?.result === 1 ? '合格' :
									checkResultValue?.result === 0 ? '不合格' : '',
							}
						});
						this.inspection_list.push(arr);
						console.log(this.inspection_list)
					}
				} else {
			...
			handleFocus3(rowIndex, index) {
				if (!this.isEdit) return
				this.searchToolFlag = false;
				this.eligibleToolFlag = false;
				this.checkResultToolFlag = true;
				this.editRowIndex = rowIndex;
				// 更新编辑的索引
				this.editingIndex = index;
			},
			...
			selectCheckResult(index) {
				const editingCategory = this.inspection_list[this.editRowIndex][this.editingIndex];
				if (editingCategory) {
					this.$set(editingCategory, [editingCategory.type], index != null ? this.checkResultSelect[index]
						.text : '')
				}
			},
			closeTool() {
			...
			inspectionConfirm(isToast = true) {
				...
				function groupByType(data) {
					return data.reduce((groupedData, rows) => {
						rows.forEach(row => {
							const id = row.id;
							if (!groupedData[id]) {
								groupedData[id] = [];
				...
				const groupedByType = groupByType(this.inspection_list.slice(0, -1));
				let kw = {
					data: groupedByType
				}
				if (this.production_obj.product_category === '刺针') {
					const checkResultList = this.inspection_list[this.inspection_list.length - 1].map(item => {
						return {
							point_id: item.point_id?.length > 0 ? item.point_id[0] : null,
							result: item[item.type] == '合格' ? 1 : item[item.type] == '不合格' ? 0 : ''
						}
					});
					kw.check_result = checkResultList;
					kw.check_inspector = this.inspector_user;
					kw.control_date = this.inspector_date;
				}
				console.log(kw)
				...
				return new Promise((rv, rj) => {
					...
					this.saveTimer = setTimeout(() => {
						api.web.dataset.call_kw({
							model: 'stock.picking',
							method: 'create_incoming_check_quality_checks',
							args: [
								[]
							],
							kwargs: kw
			...
			addCellClick() {
				let arr = this.inspection_list[this.inspection_list.length - 2].map(i => {
					return {
						...i,
						initial_value: '',
						[i.type]: '',
				...
				this.inspection_list.splice(this.inspection_list.length - 1, 0, arr);
			},
			delCellClick() {
				if (this.inspection_list.length <= 1) return;
				this.inspection_list.splice(this.inspection_list.length - 2, 1);
			},
			removeHanlde(index) {
		...
		async onLoad(options) {
			...
			this.production_obj = production_obj;
