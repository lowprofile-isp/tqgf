	...
	<view class="materialManagementScan-page" :style="{height: `calc(100vh - ${$mStore.state.navBarHeight}px)`}">
		<u-navbar :is-back="true" back-icon-color="#fff" :border-bottom="false" :title="navStyles.navTitle"
			...
			<view slot="right" class="slot-right-wrap">
				...
				<view class="right-item" style="color: #fff;" @click="is_manual = !is_manual">
					{{is_manual ? '扫码添加' : '手动添加'}}
				</view>
		...
		<view class="container">
			...
			<view class="content" v-if="!is_manual">
			...
			<view class="content" v-else>
				<view class="content-title" style="display: flex; align-items: center;">
					<uni-easyinput v-model="serial_left" class="submit-icon" :styles="easyinputStyles" type="text"
						placeholder="请输入固定的编码" trim :errorMessage="errorMessageLeft"></uni-easyinput>
					<span style="margin: 0 10rpx;">-</span>
					<uni-easyinput style="width: 170rpx; flex: none;" v-model="serial_right" class="submit-icon"
						:styles="easyinputStyles" type="number" placeholder="请输入编号" trim
						:errorMessage="errorMessageRight"></uni-easyinput>
					<u-button style="margin-left: 20rpx; width: 100rpx; height: 72rpx; line-height: 72rpx;" plain
						type="primary" @click="addManual">添加</u-button>
				</view>
				<scroll-view class="material-list" scroll-y :scroll-top="scrollTop" @scroll="scrollTap">
					<!-- <view v-if="material_list.length <= 0" class="not-empty">请添加产品</view> -->
					<uni-forms class="material-item" v-for="(item,index) in manual_material_list" :key="index"
						:modelValue="item" label-width="60">
		...
		<view>
			<view class="btn-group">
				<u-button type="error" style="flex: 1; margin-right: 20rpx;"
					:disabled="!equipmentInfo.id || (is_manual ? !manual_material_list.length : !material_list.length)"
					@click="errorClick">异常</u-button>
				<u-button type="primary" :loading="btnLoading"
					:disabled="!equipmentInfo.id || (is_manual ? !manual_material_list.length : !material_list.length)"
					@click="postEquipmentInfo">提交</u-button>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				equipmentInfo: {},
				material_list: [],
				manual_material_list: [],
				selectList: null,
				is_back: false,
				is_manual: false,
				serial_left: '',
				serial_right: '',
				errorMessageLeft: false,
				errorMessageRight: false
			}
		},
		watch: {
			serial_left(val) {
				if (val) {
					this.errorMessageLeft = false;
				}
			},
			serial_right(val) {
				if (val) {
		...
		methods: {
			postEquipmentInfo() {
				...
				let material_list = this.is_manual ? this.manual_material_list : this.material_list;
				...
				uni.showModal({
					...
					success: (e) => {
						if (e.confirm) {
							let products = material_list.map((m) => {
								return {
									...m,
			...
			async getProductionInfo(barcode) {
				let material_list = this.is_manual ? this.manual_material_list : this.material_list;
				let index = material_list.findIndex(m => m.serial_num === barcode);
				if (index > -1) {
					uni.showToast({
						title: this.is_manual ? '重复添加：' : '扫码重复：' + barcode,
						icon: 'none',
						duration: 3000
					});
					return
			...
			addManual() {
				if (!this.serial_left) {
					this.errorMessageLeft = true;
				}
				if (!this.serial_right) {
					this.errorMessageRight = true;
				}
				if (!this.serial_left || !this.serial_right) {
					return
				}
				let barcode = this.serial_left + '-' + this.serial_right;
				this.getProductionInfo(barcode);
			},
			zzbzPopupConfirm(e) {
				this.selectList = e;
			...
			async scanClick() {
				...
				await this.getEquipmentInfo('XT2005');
				// ['TQGF-XS-2024-DD-0822-001/BR355XBB-24-0823-006']
				// .forEach((item) => {
		...
		onLoad(options) {
			this.is_back = options.is_back ? true : false;
			if (options.barcode) {
