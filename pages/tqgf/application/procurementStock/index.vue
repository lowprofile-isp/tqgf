				...
				<view class="content">
					...
					<view class="list-container" v-else>
						<card-list ref="cardListRef" :loading="listLoading" :table-data="searchList" :page-size="9"
							fixed-pagination columnKey="procurementStock" @itemClick="toDetail">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				try {
					...
					const res = await api.web.dataset.call_kw({
						model: 'stock.picking',
						method: 'raw_material_inventory_list',
						args: [
							[]
						],
						kwargs: {
							purchase_order_name: this.search_word
						}
					})
			...
			toDetail(item) {
				let url = item && item.id ? `/pages/tqgf/application/procurementStock/detail?id=${item.id}` :
					'/pages/tqgf/application/procurementStock/detail'
				uni.navigateTo({
					url
				})
			},
