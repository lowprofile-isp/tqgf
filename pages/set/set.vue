<template>
	<view class="set">
		...
		<!--版本更新-->
		<!-- #ifdef APP-PLUS -->
		<oa-version-upgrade isTipsShow @tip="handleTip" @close="handleVersionUpgradeShow"
			:isShow="isVersionUpgradeShow"></oa-version-upgrade>
		<!-- #endif -->
		<!-- modal -->
		<!-- 选择颜色模态框 -->
...
<script>
	...
	export default {
		...
		onShow() {
			...
			// this.$mConstDataConfig.setList.forEach(item=>{
			// 	if(item.title != '注销账号'){
			// 		this.isPush = true
			// 	}
			// })
			// if(this.isPush){
			// 	this.$mConstDataConfig.setList.push({
			// 		content: "",
			// 		title: "注销账号",
			// 		url: ""
			// 	})
		...
		methods: {
			...
			initData() {
				...
				// #ifdef APP-PLUS
				this.setList[3].content = `当前版本 V1.0.7`;
				// #endif
			},
			// 通用跳转
			navTo(route) {
				if(!route){
					uni.showModal({
						content: '您的账号正在申请注销,注销后,您的账号将无法使用',
						success: e => {
							if (e.confirm) {
								uni.reLaunch({
