...
class Database extends JsonRequest {
	...
	static async create(master_pwd, name, lang, password, kwargs = {}) {
		const {
			demo = false, login, country_code = false, phone
		} = kwargs
		const url = '/jsonrpc'
...
class Dataset extends JsonRequest {
	...
	static async call_kw(payload) {
		...
		return await this.json_call(url2, {
			model,
			method,
			args,
			kwargs
...
class Session extends JsonRequest {
	...
	static get user_context() {
		...
		// console.log("allowed_company_ids" + allowed_company_ids)
		return {
			...context,
			allowed_company_ids
		}
	...
	static get server_version() {
		...
		return server_version_info?.length > 0 ? server_version_info[0] : {}
	}
	...
	static get_cookie(c_name) {
		var cookies = ''
		// #ifdef APP-PLUS
		cookies = plus.globalEvent.cookie ? plus.globalEvent.cookie.split('; ') : []
		// #endif
		// #ifdef H5
...
class Binary extends FileRequest {
	...
	static async _bak_upload_attachment_json({
		res_model,
		res_id,
		file
	}) {
...
class Export extends FileRequest {
	...
	static async _base(url, data2) {
		...
		return await this.file_export(url, {
			data
		})
	}
...
class Home extends FileRequest {
	...
	static async upload_file(formData) {
		const url = '/upload'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async sale_count(formData) {
		const url = '/sale_count'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async sale_type_count(formData) {
		const url = '/sale_type_count'
		const session_info = await this.json_call(url, formData)
		return session_info
	}
	static async sale_trend(formData) {
		const url = '/sale_trend'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async sale_ass_customer(formData) {
		const url = '/sale_ass_customer'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async sale_accounted(formData) {
		const url = '/sale_accounted'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT(formData) {
		const url = '/SMT'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT_actual(formData) {
		const url = '/SMT_actual'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP(formData) {
		const url = '/DIP'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_actual(formData) {
		const url = '/DIP_actual'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async ChuHuo(formData) {
		const url = '/ChuHuo'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async ChuHuo_actual(formData) {
		const url = '/ChuHuo_actual'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async shipment_count(formData) {
		const url = '/shipment_count'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async employee_count(formData) {
		const url = '/employee_count'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async mrp_customerAbnormal(formData) {
		const url = '/mrp_customerAbnormal'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async getSMTFractionDefective(formData) {
		const url = '/getSMTFractionDefective'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async getSPIFractionDefective(formData) {
		const url = '/getSPIFractionDefective'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async getDIPFractionDefective(formData) {
		const url = '/getDIPFractionDefective'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async getInspectionList(formData) {
		const url = '/getInspectionList'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT_AOI_2_rty(formData) {
		const url = '/SMT_AOI_2_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT_AOI_2_order(formData) {
		const url = '/SMT_AOI_2_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT_AOI_3_rty(formData) {
		const url = '/SMT_AOI_3_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SMT_AOI_3_order(formData) {
		const url = '/SMT_AOI_3_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI1_rty(formData) {
		const url = '/SPI1_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI1_order(formData) {
		const url = '/SPI1_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI3_rty(formData) {
		const url = '/SPI3_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI3_order(formData) {
		const url = '/SPI3_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI4_rty(formData) {
		const url = '/SPI4_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async SPI4_order(formData) {
		const url = '/SPI4_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_1_rty(formData) {
		const url = '/DIP_AOI_1_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_1_order(formData) {
		const url = '/DIP_AOI_1_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_2_rty(formData) {
		const url = '/DIP_AOI_2_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_2_order(formData) {
		const url = '/DIP_AOI_2_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_3_rty(formData) {
		const url = '/DIP_AOI_3_rty'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async DIP_AOI_3_order(formData) {
		const url = '/DIP_AOI_3_order'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_smt2(formData) {
		const url = '/operation_rate_smt2'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_smt3(formData) {
		const url = '/operation_rate_smt3'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_SPI1(formData) {
		const url = '/operation_rate_SPI1'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_SPI3(formData) {
		const url = '/operation_rate_SPI3'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_SPI4(formData) {
		const url = '/operation_rate_SPI4'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_DIP1(formData) {
		const url = '/operation_rate_DIP1'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_DIP2(formData) {
		const url = '/operation_rate_DIP2'
		const session_info = await this.json_call(url, formData)
		return session_info
	}

	static async operation_rate_DIP3(formData) {
		const url = '/operation_rate_DIP3'
		const session_info = await this.json_call(url, formData)
		return session_info
	}
	...
	static async content(payload) {
		...
		const {
			model = 'ir.attachment', id, field = 'datas'
		} = payload
		const {
			filename,
			filename_field = 'name'
