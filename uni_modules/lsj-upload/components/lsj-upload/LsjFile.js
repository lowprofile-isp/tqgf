...
export class LsjFile {
	...
	_uploadHandle(item) {
		...
		return new Promise((resolve, reject) => {
			...
			const tempFilePaths = item.path;
			pathToBase64(tempFilePaths).then(path => {
				console.log(this.option)
				api.web.upload_file({
					params: {
						file_name: item.name,
						ufile: path.replace(/^data:image\/\w+;base64,/, '')
							.replace(/^data:text\/\w+;base64,/, '').replace(
								/^data:application\/\w+;base64,/, ''),
						model: this.option.formData.model,
						res_id: this.option.formData.res_id,
						mail_active_id: this.option.formData.mail_active_id
					}
				}).then(res => {
					console.log(res)
				})
			...
			form['success'] = res => {
				if (res.statusCode == 200) {
					this.debug && console.log(
						'上传完成,微信端返回不一定是字符串，根据接口返回格式判断是否需要JSON.parse：' + res.data)
