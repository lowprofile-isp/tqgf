<template>
	<view class="bg-white">
		...
		<view class="cu-form-group padding-left-xl solid-bottom " style="margin-top: -20upx;">
			<view class="grid col-4 grid-square flex-sub" style="margin-left: -25upx;">
				<view class="bg-img" v-for="(item,index) in d_imgList" :key="index" @tap="ViewImage"
					:data-url="d_imgList[index]">
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			DelImg(e) {
				this.d_imgList.splice(e.currentTarget.dataset.index, 1); // 删除当前图片
				this.$emit("imgChange", this.d_imgList); // 变换图片
			},
		}
	}
