...
export function labelCommand(width, height, unit = 'percent') {
	...
	/**
	 * 设置条形码
	 * x, y, codetype: 字串型别, height: 设定条码高度，高度以点来表示 , readable: 设定是否列印条码码文, narrow, wide, content
	 */
	this.setBarCode = function(x, y, codetype, height, readable, narrow, wide, content) {
		x = this.toDotX(x);
		y = this.toDotY(y);
		this.tscCommand.setBar(x, y, codetype, height, readable, narrow, wide, content)
	};
	// 设置线条
	//（y%, x%, 粗细(一般0.5)， 宽度）
	this.setBar = function(x, y, width, height = 13) {
		x = this.toDotX(x);
		y = this.toDotY(y);
	...
	// 横线
	this.setBarHeng = function(x, y, width, height = 0.5) {
		x = this.toDotX(x);
		y = this.toDotY(y);
		width = (width * 8).toFixed(0);
		height = (height * 8).toFixed(0);
